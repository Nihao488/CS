...
let cars = [], peashooters = [], tallnuts = [], spikeweeds = [], cursor, camera, health = 100;
...
window.onload = function(){
  ...
  for(let i = 0; i < 5; i++) { // Create 3 spikeweeds to match tallnut count
    spikeweeds.push(new Spikeweed(-0.2, zPositions[i]));
  }
  ...
  for(let i = 0; i < 5; i++) { // Create 3 tallnuts as example
    tallnuts.push(new Tallnut(-2, 0.5, -4 + (i * 2)));
  }
...
function loop(){
  ...
  spikeweeds.forEach(spike => {
    const spikePos = spike.obj.object3D.position;
    const camPos = camera.object3D.position;
    const dx = spikePos.x - camPos.x;
    const dy = spikePos.y - camPos.y;
    const dz = spikePos.z - camPos.z;
    const dist = Math.sqrt(dx*dx + dy*dy + dz*dz);
    
    if(dist < 1) { // 1 meter collision radius
        health = Math.max(health - 2, 0);
        health_text.setAttribute('value', `Health: ${health}`);
        
        if(health <= 0) {
            gameOver = true;
            alert('GAME OVER - Impaled on spikeweed!');
            return;
        }
    }
  });
